...
proc_hmapf(hmapf_t *map, int lod, int sort_ratio)
{
  ...
  /* Filter out hmap holding fov0 in alternate passes while setting up for
     ...
     sort_proc_hmaps per frame, then again in sort. note: Soon, more than one
     hmap may hold VRT_MASK_LOD_INF, this is why matching is done on session_t.
     Now that session_t has grown hash.l will be used instead.  For now doing
     the copy, match, with the full session_t.  Will revisit soon. ... */
